...
func Test_constructPath(t *testing.T) {
	for _, tt := range testsConstructPath {
		t.Run(tt.name, func(t *testing.T) {
...
func FuzzConstructPath(f *testing.F) {
	for _, tt := range testsParsePath {
		f.Add(tt.args)
	}
	...
	f.Fuzz(func(t *testing.T, args string) {
		constructPath(parsePath(args))
	})
}

type argsPathCIA struct {
	clas     int
	instance int
	attr     int
	member   int
...
var testsPathCIA = []struct {
	name string
	args argsPathCIA
	want []uint8
}{
	{"01", argsPathCIA{PortClass, 0, -1, -1}, []uint8{0x20, 0xF4, 0x24, 0x00}},
	{"02", argsPathCIA{FileClass, 200, -1, -1}, []uint8{0x20, 0x37, 0x24, 0xC8}},
	{"03", argsPathCIA{FileClass, 2000, -1, -1}, []uint8{0x20, 0x37, 0x25, 0x00, 0xD0, 0x07}},
	{"04", argsPathCIA{2000, 2000, -1, -1}, []uint8{0x21, 0x00, 0xD0, 0x07, 0x25, 0x00, 0xD0, 0x07}},
}

func Test_pathCIA(t *testing.T) {
	for _, tt := range testsPathCIA {
		t.Run(tt.name, func(t *testing.T) {
...
func FuzzPathCIA(f *testing.F) {
	for _, tt := range testsPathCIA {
		f.Add(tt.args.clas, tt.args.instance, tt.args.attr, tt.args.member)
	}

	f.Fuzz(func(t *testing.T, clas, instance, attr, member int) {
		pathCIA(clas, instance, attr, member)
	})
}
